package com.github.yjz.widget.imageview
 ...
 * 圆形显示ImageView
 * @author lopspower
 * github地址：https://github.com/lopspower/CircularImageView/tree/master
 */
class SimpleCircleImageView @JvmOverloads constructor(
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val attributes =
            context.obtainStyledAttributes(attrs, R.styleable.SimpleCircleImageView, defStyleAttr, 0)

        // Init Background Color
        circleColor =
            attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_circle_color, Color.WHITE)
        attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_circle_color_start, 0)
            .also { if (it != 0) circleColorStart = it }
        attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_circle_color_end, 0)
            .also { if (it != 0) circleColorEnd = it }
        circleColorDirection = attributes.getInteger(
            R.styleable.SimpleCircleImageView_yjz_civ_circle_color_direction,
            circleColorDirection.value
        ).toGradientDirection()

        // Init Border
        if (attributes.getBoolean(R.styleable.SimpleCircleImageView_yjz_civ_border, true)) {
            val defaultBorderWidth =
                DEFAULT_BORDER_WIDTH * resources.displayMetrics.density
            borderWidth = attributes.getDimension(
                R.styleable.SimpleCircleImageView_yjz_civ_border_width,
                defaultBorderWidth
            )
            borderColor =
                attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_border_color, Color.WHITE)
            attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_border_color_start, 0)
                .also { if (it != 0) borderColorStart = it }
            attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_border_color_end, 0)
                .also { if (it != 0) borderColorEnd = it }
            borderColorDirection = attributes.getInteger(
                R.styleable.SimpleCircleImageView_yjz_civ_border_color_direction,
        ...
        // Init Shadow
        shadowEnable =
            attributes.getBoolean(R.styleable.SimpleCircleImageView_yjz_civ_shadow, shadowEnable)
        if (shadowEnable) {
            shadowGravity = attributes.getInteger(
                R.styleable.SimpleCircleImageView_yjz_civ_shadow_gravity,
                shadowGravity.value
            ).toShadowGravity()
            val defaultShadowRadius = DEFAULT_SHADOW_RADIUS * resources.displayMetrics.density
            shadowRadius = attributes.getDimension(
                R.styleable.SimpleCircleImageView_yjz_civ_shadow_radius,
                defaultShadowRadius
            )
            shadowColor =
                attributes.getColor(R.styleable.SimpleCircleImageView_yjz_civ_shadow_color, shadowColor)
        }
